...
export interface PaperXcelTransport {
  invoke: typeof import("electron").ipcRenderer.invoke;
  on: typeof import("electron").ipcRenderer.on;
  removeListener: typeof import("electron").ipcRenderer.removeListener;
  getPathForFile(file: File): string;
}

export function createPaperXcelApi(
  ipcRenderer: PaperXcelTransport,
): PaperXcelApi {
  const webUtils = { getPathForFile: ipcRenderer.getPathForFile };
  const api: PaperXcelApi = {
    ...
    notes: {
      ...
      getDraft: (paperId: string): Promise<PaperNote | null> =>
        ipcRenderer.invoke("notes:get-draft", paperId),
      ...
      onProgress: (listener: (event: PaperNoteStream) => void) => {
        const wrapped = (
          _event: Electron.IpcRendererEvent,
          progress: PaperNoteStream,
        ): void => listener(progress);
        ipcRenderer.on("notes:progress", wrapped);
        return () => ipcRenderer.removeListener("notes:progress", wrapped);
      },
      onAgentEvent: (listener: (event: AgentEvent) => void) => {
        const wrapped = (
          _event: Electron.IpcRendererEvent,
      ...
      onMarkdownPreview: (
        ...
        return () =>
          ipcRenderer.removeListener(
            "knowledge-base:markdown-preview",
            handler,
          );
      },
    },
    ...
    citationGraph: {
      listResearchConversations: (kind) =>
        ipcRenderer.invoke("citation-graph:research-list", kind),
      getResearchConversation: (id) =>
        ipcRenderer.invoke("citation-graph:research-get", id),
      renameResearchConversation: (id, title) =>
        ipcRenderer.invoke("citation-graph:research-rename", id, title),
      deleteResearchConversation: (id) =>
        ipcRenderer.invoke("citation-graph:research-delete", id),
      deleteResearchTurn: (id, turnId) =>
        ipcRenderer.invoke("citation-graph:research-delete-turn", id, turnId),
      discoverWithAi: (input) =>
        ipcRenderer.invoke("citation-graph:discover-with-ai", input),
      cancelDiscoveryAgent: (id) =>
        ipcRenderer.invoke("citation-graph:cancel-discovery-agent", id),
      onDiscoveryAgentProgress: (listener) => {
        const handler = (_event: unknown, progress: ChatProgress) =>
          listener(progress);
        ipcRenderer.on("citation-graph:discovery-agent-progress", handler);
        return () =>
          ipcRenderer.removeListener(
            "citation-graph:discovery-agent-progress",
            handler,
          );
      },
      onDiscoveryAgentEvent: (listener) => {
        const handler = (_event: unknown, event: AgentEvent) => listener(event);
        ipcRenderer.on("citation-graph:discovery-agent-event", handler);
        return () =>
          ipcRenderer.removeListener(
            "citation-graph:discovery-agent-event",
            handler,
          );
      },
      cancelDiscovery: (requestId: string) =>
        ipcRenderer.invoke("citation-graph:cancel-discovery", requestId),
      onDiscoveryProgress: (
        listener: (progress: CitationDiscoveryProgress) => void,
      ) => {
        const handler = (
          _event: unknown,
          progress: CitationDiscoveryProgress,
        ): void => listener(progress);
        ipcRenderer.on("citation-graph:discovery-progress", handler);
        return () =>
          ipcRenderer.removeListener(
            "citation-graph:discovery-progress",
            handler,
          );
      },
      ...
      references: (
        ...
        ipcRenderer.invoke("citation-graph:references", input),
      refresh: (
        force = false,
        paperIds?: string[],
      ...
      analyze: (
        ...
        ipcRenderer.invoke("citation-graph:analyze", input),
      cancelAnalysis: (requestId: string): Promise<boolean> =>
        ipcRenderer.invoke("citation-graph:cancel-analysis", requestId),
      onAnalysisProgress: (listener: (progress: ChatProgress) => void) => {
        const handler = (_event: unknown, progress: ChatProgress) =>
          listener(progress);
        ipcRenderer.on("citation-graph:analysis-progress", handler);
        return () =>
          ipcRenderer.removeListener(
            "citation-graph:analysis-progress",
            handler,
          );
      },
      onAnalysisEvent: (listener: (event: AgentEvent) => void) => {
        const handler = (_event: unknown, event: AgentEvent) => listener(event);
        ipcRenderer.on("citation-graph:analysis-event", handler);
        return () =>
          ipcRenderer.removeListener("citation-graph:analysis-event", handler);
      },
